	...
	if(!mysqli_stmt_prepare($stmt, $report_id)){
		...
		if(!mysqli_stmt_prepare($stmt, $d_created)){
			...
			if(isset($_POST['assignedTo'])){
				...
				// IF THE USER UNDO (UPDATE THE ISSUE ROW IN THE DATABASE)
				if(isset($_POST['new']))
                {
					if(is_null($issue_desc) || $issue_desc == ""){
                        $query = "UPDATE `issue` SET issue = '".$issue."', machine_id = '".$e_id."', date_due = '".$dueDate."', issue description = '".$issue_desc."', assigned_to = '".$assignedTo."' WHERE issue_id = '".$_POST['new']."'";
                    }else{
                        $query = "UPDATE `issue` SET issue = '".$issue."', machine_id = '".$e_id."', date_due = '".$dueDate."', assigned_to = '".$assignedTo."' WHERE issue_id = '".$_POST['new']."'";
                    }

					//echo $issue. $e_id. $dueDate. $issue_desc. $assignedTo. "/".$_POST['new'];
                    ...
                    header("Location: ../assign_issue.php?site=Create%20Issue%20Report&status=undo&id=".$_POST['new']."");
                    exit();
					}
                ...
                {
					if(is_null($issue_desc) || $issue_desc == ""){
                        $query = "UPDATE `issue` SET issue = '".$issue."', machine_id = '".$e_id."', issue description = '".$issue_desc."' WHERE issue_id = '".$_POST['new']."'";
                    }else{
                        $query = "UPDATE `issue` SET issue = '".$issue."', machine_id = '".$e_id."',  WHERE issue_id = '".$_POST['new']."'";
                    }
				...
				else
                {
                    ...
                    if(!mysqli_stmt_prepare($stmt, $query)){
                        ...
                        if(!mysqli_stmt_prepare($stmt, $equip)){
                            ...
                            header("Location: ../assign_issue.php?site=Create%20Issue%20Report&status=submitted&id=".$report_id."");
                            exit();
                        }
