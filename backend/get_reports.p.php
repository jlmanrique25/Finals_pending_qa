...
<?php
	...
	ORDER BY ".$order." ".$by." LIMIT ".$min.",10";
			...
			$sql_dates = "SELECT * FROM `reports`, `equipment`, `location`, `users` WHERE reports.date_created BETWEEN '".$_GET['s']."'AND '".$_GET['e']."' AND reports.machine_id = equipment.equipment_id AND location.location_id = equipment.location_id  AND users.users_id = assigned_user LIMIT ".$min.",10";
		}else if($_GET['time'] == 'day'){
			
			$sql_dates = "SELECT report_id, task, machine_id, task_due, date_submitted, report_status, for_repair, assigned_user, dates.date_identity,dates.date_type, dates.report_issue_id, date_time, equipment.equipment_name, equipment.equipment_id, location.location_id, location.floor, location.room_number, reports.report_status, users.username, users.users_id, date_created
			FROM `dates`,`reports`, `equipment`, `location`, `users`  
			WHERE ".$_GET['time']."(date_time) = ".$_GET['time']."(now()) AND day(date_time) = day(now()) AND month(date_time) = month(now()) AND date_identity = 'report' AND date_type = 'created' AND dates.report_issue_id = reports.report_id AND reports.machine_id = equipment.equipment_id AND location.location_id = equipment.location_id  AND YEAR(date_time) = year(now()) AND users.users_id = assigned_user
			ORDER BY ".$order." ".$by." LIMIT ".$min.",10";
		...
		if(!isset($_GET['time'])){
			$sql_dates = "SELECT issue_id, machine_id, issue, issue_status, date_due, date_created, date_issue_resolved, assigned_to, equipment.equipment_id, equipment.equipment_name, users.users_id, users.username, dates.date_time, dates.report_issue_id, dates.date_identity FROM `issue`, `equipment`, `users`, `dates` WHERE day(dates.date_time) = day(now()) AND year(date_time) = year(now()) AND date_type = 'created' AND issue.machine_id = equipment.equipment_id AND assigned_to = users.users_id ORDER by ".$order." ".$by." LIMIT ".$min.",10";
		}else if($_GET['time'] == 'date'){
			...
			$sql_dates = "SELECT issue_id, machine_id, issue, issue_status, date_due, date_created, date_issue_resolved, assigned_to, equipment.equipment_id, equipment.equipment_name, users.users_id, users.username, dates.date_time, dates.report_issue_id, dates.date_identity FROM `issue`, `equipment`, `users`, `dates` WHERE ".$_GET['time']."(dates.date_time) = ".$_GET['time']."(now()) AND date_type = 'created' AND year(date_time) = year(now()) AND dates.report_issue_id = issue.issue_id AND date_identity = 'issue' AND issue.machine_id = equipment.equipment_id AND assigned_to = users.users_id ORDER by ".$order." ".$by." LIMIT ".$min.",10";
		}
